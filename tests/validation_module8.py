...
def test_biophysics_round1_stability():
    ...
    output = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "biophysics_r1_output.pdb", stiffness=0.0
    )
...
def test_biophysics_round2_convergence():
    ...
    out1 = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "biophysics_r2_first.pdb", stiffness=0.0
    )

    logger.info("Second minimization run (should show minimal change)...")
    out2 = minimizer.minimize(
        out1, output_path=OUTPUT_DIR / "biophysics_r2_second.pdb", stiffness=0.0
    )
...
def test_structural_round1_integrity():
    ...
    output = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "structural_r1_minimized.pdb", stiffness=0.0
    )
...
def test_structural_round3_pocket_preservation():
    ...
    out_loose = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "structural_r3_loose.pdb", stiffness=0.0
    )
    ...
    out_tight = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "structural_r3_tight.pdb", stiffness=500.0
    )
    ...
    assert rmsd_tight < 0.5, f"Restrained RMSD too high: {rmsd_tight:.4f} Å (expected < 0.5 Å)"
...
def test_biochem_round1_docking_competence():
    ...
    minimized_receptor = minimizer.minimize(
        TEST_PDB_ALTERNATIVE, output_path=OUTPUT_DIR / "biochem_r1_receptor.pdb", stiffness=10.0
    )
